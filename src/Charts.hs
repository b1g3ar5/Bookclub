...
writeMeannessChart tdb = do
    ...
    let nys =  map snd ncols
    let maxs = map mmax nys
    let mins = map mmin nys
    let means = map mmean nys
    let stds = map mstd nys
...
let bs = map (pickerBox tdb) pickers
let opens = map bopen bs
let maxs = map bhi bs
let mins = map blo bs
let closes = map bclose bs
...
writeChartStat name ys = "<row><string>"
                      ++ name
                      ++ "</string>\n"
